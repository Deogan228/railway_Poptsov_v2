...
object IdInterpreters:
  ...
  // тут уже настоящие println и readline
  given Console[Id] with
    def writeLine(s: String): Unit = println(s)
    def write(s: String): Unit     = print(s)
    def readLine: String =
      val s = scala.io.StdIn.readLine()
      if s == null then "" else s //на случай, если вход закрыт

  // лог хранится в изменяемом буфере, take забирает всё и очищает
  ...
  // счетчик id для билетов, просто инкрементируем
  ...
  // флаг открытости кассы, просто булево значение
  ...
  class IdTrainRepo(initial: List[Train] = Nil) extends TrainRepo[Id]:
    ...
    def setSeat(trainName: String, seat: String, occupied: Boolean): Unit = //находим поезд, обновляем его место, сохраняем обратно
      storage.get(trainName).foreach { t =>
        storage.update(trainName, t.copy(seats = t.seats.updated(seat, occupied)))
      }

  // реализация TicketRepo
